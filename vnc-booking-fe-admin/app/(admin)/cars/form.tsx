  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await fetch(
        documentId ? `/api/cars/${documentId}` : `/api/cars`,
        {
      ...
      setTimeout(() => { router.push("/cars"); }, 600);
    } catch (err: any) {
  ...
  return (
    <Card className="w-full max-w-2xl mx-auto">
      <CardHeader>
        ...
        <CardDescription>
          {isEditing ? "Edit your car rental listing." : "Add a new car rental listing."}
        </CardDescription>
      </CardHeader>
      <CardContent>
        <form onSubmit={onSubmit} className="grid gap-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="grid gap-2">
              ...
              <Select key={`transmission-select-${selectResetKey}`} value={transmission} onValueChange={handleTransmissionChange}>
                <SelectTrigger id="transmission" aria-label="Transmission">
                  <SelectValue placeholder="Select transmission" />
                </SelectTrigger>
                <SelectContent position="popper" side="bottom" align="start" className="z-50 min-w-[8rem]" sideOffset={4} avoidCollisions={true} sticky="partial">
          ...
          <div className="grid gap-2">
            ...
            <Textarea id="description" name="description" placeholder="Write the car details..." defaultValue={car?.description as any} />
          </div>
