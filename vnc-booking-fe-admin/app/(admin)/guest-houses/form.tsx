  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await fetch(
        documentId ? `/api/guest-houses/${documentId}` : `/api/guest-houses`,
        {
      ...
      setTimeout(() => { router.push("/guest-houses"); }, 600);
    } catch (err: any) {
  ...
  return (
    <Card className="w-full max-w-2xl mx-auto">
      <CardHeader>
        ...
        <CardDescription>
          {isEditing ? "Edit your guest house listing." : "Add a new guest house listing."}
        </CardDescription>
      </CardHeader>
      <CardContent>
        <form onSubmit={onSubmit} className="grid gap-4">
          <div className="grid gap-2">
            ...
            <Input id="title" name="title" defaultValue={guestHouse?.title} required />
          </div>
          <div className="grid gap-2">
            <Label htmlFor="location">Location</Label>
            <Input id="location" name="location" defaultValue={guestHouse?.location} required />
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="grid gap-2">
              ...
              <Input id="price" name="price" type="number" step="0.01" defaultValue={guestHouse?.price} required />
            </div>
            <div className="grid gap-2">
              <Label htmlFor="rating">Rating (0-5)</Label>
              <Input id="rating" name="rating" type="number" step="0.1" min="0" max="5" defaultValue={guestHouse?.rating} required />
            </div>
          </div>
          <div className="grid gap-2">
            <Label htmlFor="description">Description</Label>
            <Textarea id="description" name="description" placeholder="Write the guest house details..." defaultValue={guestHouse?.description as any} />
          </div>
          <Button type="submit" disabled={loading}>
            {loading ? "Saving..." : isEditing ? "Update Guest House" : "Create Guest House"}
          </Button>
          {message && (
            <p className={`text-sm ${/successfully/i.test(message) ? "text-green-600" : "text-red-500"}`}>
              {message}
            </p>
          )}
        </form>
